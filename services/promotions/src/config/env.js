...
const devSchema = baseSchema.extend({
  ...
  PRICES_HOST_DEV: z.string().min(1),
  PRICES_PORT_DEV: z.coerce.number(),
  PRICES_VER_DEV: z.string().min(1),
});
...
const prodSchema = baseSchema.extend({
  ...
  PRICES_HOST_PROD: z.string().min(1),
  PRICES_PORT_PROD: z.coerce.number(),
  PRICES_VER_PROD: z.string().min(1),
});
